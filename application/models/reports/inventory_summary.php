...
class Inventory_summary extends Report
{
	...
	public function getSummaryData(array $inputs)
	{
		...
		$this->db->select('sum('.$this->db->dbprefix('item_quantities').'.quantity) as total_quantity,sum(cost_price) as total_cost_price,sum(unit_price) as total_unit_price,sum(whole_price) as total_whole_price');
		$this->db->where('items.deleted', 0);	
